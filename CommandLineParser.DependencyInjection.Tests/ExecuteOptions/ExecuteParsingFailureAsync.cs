...
class ExecuteParsingFailureAsync : IExecuteParsingFailureAsync<string>
{
    ...
    public Task<string> ExecuteAsync(string[] args, IEnumerable<Error> errors, CancellationToken ctx = default) => 
        Task.FromResult($"Unable to parse \"{string.Join(' ', args)}\" ASYNC.");
